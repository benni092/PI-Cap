...
3= Install PiCap for USB Capturecards with live preview on desktop
4= Uninstall PiCap
" userselect
...
sudo apt update
sudo apt -y install libgstreamer1.0-dev libgstreamer-plugins-base1.0-dev libgstreamer-plugins-bad1.0-dev gstreamer1.0-plugins-ugly gstreamer1.0-plugins-good gstreamer1.0-plugins-bad gstreamer1.0-tools gstreamer1.0-gl gstreamer1.0-gtk3 gstreamer1.0-alsa wmctrl xterm gpiod
if [ $? = 0 ]
then
if [ -d /home/$USER/cap ]
then
...
if [ -e /home/$USER/cap_usb_live.sh ]
then
echo "file cap_usb_live.sh already there, skipping"
else
cat > /home/$USER/cap_usb_live.sh <<EOF
#!/bin/bash
#v4l2-ctl -c brightness=0 &&
#v4l2-ctl -c contrast=148 &&
#v4l2-ctl -c saturation=180 &&
...
EOF
chmod 777 /home/$USER/cap_usb_live.sh
fi
if [ -e /home/$USER/.local/share/applications/Pi-Cap-live.desktop ]
then
echo "file Pi-Cap-live.desktop already there, skipping"
else
cat > /home/$USER/.local/share/applications/Pi-Cap-live.desktop <<EOF
[Desktop Entry]
Comment=
Terminal=false
Name=cap_usb_live
Exec=xterm -fa ’Monospace’ -fs 20 -geometry 4x2+0+0 -e /home/$USER/cap_usb_live.sh
Type=Application
...
read -p "Do you want to autostart? " test
if [ $test = y ]; then
if [ $userselect = 1 ]; then
